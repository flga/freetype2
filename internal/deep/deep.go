...
// to when comparing.
func WithFloatPrecision(v int) func(*config) { return func(c *config) { c.floatPrecision = v } }

// WithMaxDiff specifies the maximum number of differences to return.
func WithMaxDiff(v int) func(*config) { return func(c *config) { c.maxDiff = v } }

// WithMaxDepth specifies the maximum levels of a struct to recurse into,
// if greater than zero. If zero, there is no limit.
func WithMaxDepth(v int) func(*config) { return func(c *config) { c.maxDepth = v } }

// WithLogErrors causes errors to be logged to STDERR.
func WithLogErrors() func(*config) { return func(c *config) { c.logErrors = true } }

// WithoutLogErrors causes errors to not be logged.
func WithoutLogErrors() func(*config) { return func(c *config) { c.logErrors = false } }

// WithCompareUnexportedFields causes unexported struct fields, like s in
// T{s int}, to be compared.
func WithCompareUnexportedFields() func(*config) {
	return func(c *config) { c.compareUnexportedFields = true }
}

// WithoutCompareUnexportedFields causes unexported struct fields, like s in
// T{s int}, to be ignored.
func WithoutCompareUnexportedFields() func(*config) {
	return func(c *config) { c.compareUnexportedFields = false }
}
...
// ignored.
func Equal(a, b interface{}, options ...option) []string {
	conf := &config{
	...
	c := &cmp{
		...
		diff:        []string{},
		buff:        []string{},
		floatFormat: fmt.Sprintf("%%.%df", conf.floatPrecision),
	}
	if a == nil && b == nil {
		return nil
	} else if a == nil && b != nil {
...
func (c *cmp) equals(a, b reflect.Value, level int) {
	if c.config.maxDepth > 0 && level > c.config.maxDepth {
	...
	case reflect.Struct:
		...
		*/
		...
		for i := 0; i < a.NumField(); i++ {
			...
			if len(c.diff) >= c.config.maxDiff {
				break
			}
		}
	case reflect.Map:
