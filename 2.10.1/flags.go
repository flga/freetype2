...
//
// By default, hinting is enabled and the font's native hinter (see FaceFlagHinter)
// is preferred over the auto-hinter. You can disable hinting by setting
// LoadNoHinting or change the precedence by setting LoadForceAutohint. You can
// also set LoadNoAutohint in case you don't want the auto-hinter to be used at all.
//
// See the description of FaceFlagTricky for a special exception (affecting only
// a handful of Asian fonts).
//
// Besides deciding which hinter to use, you can also decide which hinting
// algorithm to use. See LoadTarget for details.
//
...
// incorrect mapping (for example, assigning the character code U+005A,
// LATIN CAPITAL LETTER Z, to a glyph depicting a mathematical integral sign),
...
const (
	// LoadDefault is used as the default glyph load operation. In this case,
	...
	// be accessed from the glyph slot (see the note in LoadFlag).
	// 2 - If no embedded bitmap is searched for or found, FreeType looks for
	// a scalable outline. If one is found, it is loaded from the font file,
	// scaled to device pixels, then ‘hinted’ to the pixel grid in order to
	// optimize it. The outline data can be accessed from the glyph slot
	// (see the note in LoadFlag).
	//
	// Note that by default the glyph loader doesn't render outlines into bitmaps.
	// The following flags are used to modify this default behaviour to more
	// specific and useful cases.
	...
	//
	// If the font is ‘tricky’ (see FaceFlagTricky for more), using LoadNoScale
	// usually yields meaningless outlines because the subglyphs must be scaled
	// and positioned with hinting instructions. This can be solved by loading
	// the font without LoadNoScale and setting the character size to face.UnitsPerEM().
	LoadNoScale LoadFlag = C.FT_LOAD_NO_SCALE
	// LoadNoHinting disables hinting. This generally generates ‘blurrier’ bitmap
	// glyphs when the glyph are rendered in any of the anti-aliased modes.
	// See also the note in LoadFlag.
	//
	// This flag is implied by LoadNoScale.
	LoadNoHinting LoadFlag = C.FT_LOAD_NO_HINTING
	// LoadRender call RenderGlyph after the glyph is loaded. By default, the
	// glyph is rendered in RenderModeNormal mode. This can be overridden by any
	// LoadTarget or LoadMonochrome.
	//
	// This flag is unset by LoadNoScale.
	LoadRender LoadFlag = C.FT_LOAD_RENDER
	// LoadNoBitmap ignores bitmap strikes when loading. Bitmap-only fonts ignore
	// this flag.
	//
	// LoadNoScale always sets this flag.
	LoadNoBitmap LoadFlag = C.FT_LOAD_NO_BITMAP
	// LoadVerticalLayout load the glyph for vertical text layout. In particular,
	// the advance value in the GlyphSlot is set to the VertAdvance value of the
	// metrics field.
	//
	// If the face does not have FaceFlagVertical, you shouldn't use this flag
	// currently. Reason is that in this case vertical metrics get synthesized,
	...
	// LoadForceAutohint prefer the auto-hinter over the font's native hinter.
	// See also the note in LoadFlag.
	LoadForceAutohint LoadFlag = C.FT_LOAD_FORCE_AUTOHINT
	// LoadPedantic makes the font driver perform pedantic verifications during
	// glyph loading and hinting. This is mostly used to detect broken glyphs in
	// fonts. By default, FreeType tries to handle broken fonts also.
	//
	// In particular, errors from the TrueType bytecode engine are not passed to
	// the application if this flag is not set; this might result in partially
	// hinted or distorted glyphs in case a glyph's bytecode is buggy.
	LoadPedantic LoadFlag = C.FT_LOAD_PEDANTIC
	// LoadNoRecurse don't load composite glyphs recursively. Instead, the font
	// driver fills the NumSubglyph and Subglyphs values of the glyph slot; it
	// also sets glyph.Format to GlyphFormatComposite. The description of subglyphs
	// can then be accessed with GetSubGlyphInfo.
	//
	// Don't use this flag for retrieving metrics information since some font
	// drivers only return rudimentary data.
	//
	...
	// LoadMonochrome is used with LoadRender to indicate that you want to render
	// an outline glyph to a 1-bit monochrome bitmap glyph, with 8 pixels packed
	// into each byte of the bitmap data.
	//
	// Note that this has no effect on the hinting algorithm used. You should
	// rather use LoadTargetMono so that the monochrome-optimized hinting
	// algorithm is used.
	LoadMonochrome LoadFlag = C.FT_LOAD_MONOCHROME
	// LoadLinearDesign keep LinearHoriAdvance and LinearVertAdvance fields of
	// GlyphSlot in font units. See GlyphSlot for details.
	LoadLinearDesign LoadFlag = C.FT_LOAD_LINEAR_DESIGN
	// LoadNoAutohint disables the auto-hinter. See also the note in LoadFlag.
	LoadNoAutohint LoadFlag = C.FT_LOAD_NO_AUTOHINT
	// LoadColor loads colored glyphs. There are slight differences depending on
	// the font format.
	//
	// [Since 2.5] Load embedded color bitmap images. The resulting color bitmaps,
	// if available, will have the PixelModeBGRA format, with pre-multiplied color
	// channels. If the flag is not set and color bitmaps are found, they are
	// converted to 256-level gray bitmaps, using the PixelModeGray format.
	//
	// [Since 2.10, experimental] If the glyph index contains an entry in the
	// face's ‘COLR’ table with a ‘CPAL’ palette table (as defined in the
	// OpenType specification), make RenderGlyph provide a default blending of
	// the color glyph layers associated with the glyph index, using the same
	// bitmap format as embedded color bitmap images. This is mainly for
	// convenience; for full control of color layers use GetColorGlyphLayer and
	// FreeType's color functions like PaletteSelect instead of setting LoadColor
	// for rendering so that the client application can handle blending by itself.
	LoadColor LoadFlag = C.FT_LOAD_COLOR
	// LoadComputeMetrics [Since 2.6.1] Compute glyph metrics from the glyph
	// data, without the use of bundled metrics tables (for example, the ‘hdmx’
	// table in TrueType fonts). This flag is mainly used by font validating or
	// font editing applications, which need to ignore, verify, or edit those
	// tables.
	//
	// Currently, this flag is only implemented for TrueType fonts.
	LoadComputeMetrics LoadFlag = C.FT_LOAD_COMPUTE_METRICS
	// LoadBitmapMetricsOnly [Since 2.7.1] request loading of the metrics and
	// bitmap image information of a (possibly embedded) bitmap glyph without
	// allocating or copying the bitmap image data itself. No effect if the target
...
const (
	// FaceFlagScalable the face contains outline glyphs. Note that a face can
	// contain bitmap strikes also, i.e., a face can have both this flag and
	// FaceFlagFixedSizes set.
	FaceFlagScalable FaceFlag = C.FT_FACE_FLAG_SCALABLE

	// FaceFlagFixedSizes the face contains bitmap strikes. See also the
	// Face.NumFixedSizes() and Face.AvailableSizes().
	FaceFlagFixedSizes FaceFlag = C.FT_FACE_FLAG_FIXED_SIZES

	// FaceFlagFixedWidth the face contains fixed-width characters (like Courier,
	// Lucida, MonoType, etc.).
	FaceFlagFixedWidth FaceFlag = C.FT_FACE_FLAG_FIXED_WIDTH

	// FaceFlagSfnt the face uses the SFNT storage scheme. For now, this means
	// TrueType and OpenType.
	FaceFlagSfnt FaceFlag = C.FT_FACE_FLAG_SFNT

	// FaceFlagHorizontal the face contains horizontal glyph metrics. This should
	// be set for all common formats.
	FaceFlagHorizontal FaceFlag = C.FT_FACE_FLAG_HORIZONTAL

	// FaceFlagVertical the face contains vertical glyph metrics. This is only
	// available in some formats, not all of them.
	FaceFlagVertical FaceFlag = C.FT_FACE_FLAG_VERTICAL

	// FaceFlagKerning the face contains kerning information. If set, the kerning
	// distance can be retrieved using the GetKerning() method.
	// Otherwise the function always returnS the vector (0,0). Note that FreeType
	// doesn't handle kerning data from the SFNT ‘GPOS’ table (as present in many
	// OpenType fonts).
	FaceFlagKerning FaceFlag = C.FT_FACE_FLAG_KERNING

	// FaceFlagMultipleMasters the face contains multiple masters and is capable
	...
	// FaceFlagGlyphNames the face contains glyph names, which can be retrieved
	// using GetGlyphName().
	//
	// Note that some TrueType fonts contain broken glyph name tables.
	// Use HasPSGlyphNames() when needed.
	FaceFlagGlyphNames FaceFlag = C.FT_FACE_FLAG_GLYPH_NAMES

	// FaceFlagHinter the font driver has a hinting machine of its own. For example,
	// with TrueType fonts, it makes sense to use data from the SFNT ‘gasp’ table
	// only if the native TrueType hinting engine (with the bytecode interpreter)
	// is available and active.
	FaceFlagHinter FaceFlag = C.FT_FACE_FLAG_HINTER

	// FaceFlagCidKeyed The face is CID-keyed. In that case, the face is not
	// accessed by glyph indices but by CID values.
	// For subsetted CID-keyed fonts this has the consequence that not all index
	// values are a valid argument to LoadGlyph(). Only the CID values for which
	// corresponding glyphs in the subsetted font exist make LoadGlyph() return
	// successfully; in all other cases you get an ErrInvalidArgument error.
	//
	// Note that CID-keyed fonts that are in an SFNT wrapper (this is, all
	// OpenType/CFF fonts) don't have this flag set since the glyphs are accessed
	// in the normal way (using contiguous indices); the ‘CID-ness’ isn't visible
	// to the application.
	FaceFlagCidKeyed FaceFlag = C.FT_FACE_FLAG_CID_KEYED

	// FaceFlagTricky the face is ‘tricky’, this is, it always needs the font
	// format's native hinting engine to get a reasonable result.
	// A typical example is the old Chinese font mingli.ttf (but not mingliu.ttc)
	// that uses TrueType bytecode instructions to move and scale all of its subglyphs.
	//
	// It is not possible to auto-hint such fonts using LoadForceAutohint; it will
	// also ignore LoadNoHinting. You have to set both LoadNoHinting and
	// LoadNoAutohint to really disable hinting; however, you probably never want
	// this except for demonstration purposes.
	//
	// Currently, there are about a dozen TrueType fonts in the list of tricky
	// fonts; they are hard-coded in file ttobjs.c.
	FaceFlagTricky FaceFlag = C.FT_FACE_FLAG_TRICKY

	// FaceFlagColor the face has color glyph tables. See LoadColor for more
	// information.
	FaceFlagColor FaceFlag = C.FT_FACE_FLAG_COLOR

	// FaceFlagVariation [Since 2.9] set if the current face (or named instance)
	// has been altered with SetMMDesignCoordinates, SetVarDesignCoordinates, or
	// SetVarBlend_Coordinates.
