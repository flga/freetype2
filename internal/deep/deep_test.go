...
func TestOptionsFloatPrecision(t *testing.T) {
	defaultFloatPrecision := deep.FloatPrecision
	deep.FloatPrecision = 999
	defer func() { deep.FloatPrecision = defaultFloatPrecision }()
	...
	diff = deep.Equal(1.123, 1.124, deep.WithFloatPrecision(2))
	if len(diff) > 0 {
		t.Error("should be equal:", diff)
	}
}

func TestOptionsMaxDiff(t *testing.T) {
	defaultMaxDiff := deep.MaxDiff
	deep.MaxDiff = 999
	defer func() { deep.MaxDiff = defaultMaxDiff }()
	...
	maxDiff := 3
	diff := deep.Equal(a, b, deep.WithMaxDiff(maxDiff))
	if diff == nil {
		t.Fatal("no diffs")
	}
	if len(diff) != maxDiff {
		t.Errorf("got %d diffs, expected %d", len(diff), maxDiff)
	}
}
...
func TestOptionsMaxDepth(t *testing.T) {
	...
	// No diffs because MaxDepth=2 prevents seeing the diff at 3rd level down
	diff := deep.Equal(foo, bar, deep.WithMaxDepth(2))
	if diff != nil {
		t.Errorf("got %d diffs, expected none: %v", len(diff), diff)
	}

	diff = deep.Equal(foo, bar, deep.WithMaxDepth(4))
	if diff == nil {
		t.Fatal("no diff")
	}
	if len(diff) != 1 {
...
func TestOptionsCompareUnexportedFields(t *testing.T) {
	defaultCompareUnexportedFields := deep.CompareUnexportedFields
	...
	s1 := s{a: 1, B: 2}
	s2 := s{a: 2, B: 2}
	diff := deep.Equal(s1, s2, deep.WithCompareUnexportedFields())
	if diff == nil {
		t.Fatal("no diff")
	}
	if len(diff) != 1 {
	...
	diff = deep.Equal(s1, s2, deep.WithoutCompareUnexportedFields())
	if diff != nil {
...
func TestMultipleOptions(t *testing.T) {
	...
	defer func() {
		deep.CompareUnexportedFields = defaultCompareUnexportedFields
		deep.FloatPrecision = defaultFloatPrecision
	}()
	...
	want = []string{"a: 0.0078125 != 0.00390625"}
	diff = deep.Equal(s1, s2, deep.WithCompareUnexportedFields(), deep.WithFloatPrecision(3))
	if !reflect.DeepEqual(diff, want) {
		t.Errorf("diff = %v, want %v", diff, want)
	}
}
