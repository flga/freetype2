// +build 386

package freetype2

func goRegularGlyphBuf(idx GlyphIndex, deg float64) []byte {
	switch {
	case idx == 0x24 && deg == 0:
		return []byte{
			0x00, 0x00, 0x00, 0x35, 0xff, 0x90, 0x00, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x8e, 0xff, 0xe5, 0x02, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x02, 0xe5, 0xc4, 0xff, 0x40, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x41, 0xfe, 0x27, 0xf4, 0x98, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x9b, 0xc5, 0x00, 0xa8, 0xeb, 0x04, 0x00, 0x00,
			0x00, 0x06, 0xee, 0x6f, 0x00, 0x53, 0xff, 0x48, 0x00, 0x00,
			0x00, 0x4e, 0xfd, 0x1a, 0x00, 0x09, 0xf3, 0xa0, 0x00, 0x00,
			0x00, 0xa7, 0xff, 0xfc, 0xfc, 0xfc, 0xff, 0xf0, 0x07, 0x00,
			0x0b, 0xf5, 0x75, 0x28, 0x28, 0x28, 0x61, 0xff, 0x50, 0x00,
			0x5a, 0xf6, 0x0e, 0x00, 0x00, 0x00, 0x04, 0xe9, 0xa8, 0x00,
			0xb3, 0xa7, 0x00, 0x00, 0x00, 0x00, 0x00, 0x8f, 0xf5, 0x0b,
		}
	case idx == 0x24 && deg == 90:
		return []byte{
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0b,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x07, 0x50, 0xa8, 0xf5,
			0x00, 0x00, 0x00, 0x00, 0x04, 0x48, 0xa0, 0xf0, 0xff, 0xe9, 0x8f,
			0x00, 0x02, 0x40, 0x98, 0xeb, 0xff, 0xf3, 0xff, 0x61, 0x04, 0x00,
			0x90, 0xe5, 0xff, 0xf4, 0xa8, 0x52, 0x08, 0xfc, 0x28, 0x00, 0x00,
			0xff, 0xff, 0xc4, 0x27, 0x00, 0x00, 0x00, 0xfc, 0x28, 0x00, 0x00,
			0x35, 0x8f, 0xe5, 0xfe, 0xc5, 0x6e, 0x1a, 0xfc, 0x28, 0x00, 0x00,
			0x00, 0x00, 0x02, 0x42, 0x9b, 0xee, 0xfd, 0xff, 0x75, 0x0e, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x4e, 0xa7, 0xf5, 0xf6, 0xa7,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0b, 0x5a, 0xb3,
		}
	default:
		panic("no data")
	}
}

func bungeeColorMacGlyphBuf(idx GlyphIndex) []byte {
	switch idx {
	case 0x2b:
		return []byte{
			,
		}
	default:
		panic("no data")
	}
}

func notoSansJpRegGlyphBuf(idx GlyphIndex) []byte {
	switch idx {
	case 0x22:
		return []byte{
			0x00, 0x00, 0x00, 0xa4, 0xff, 0x20, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x05, 0xef, 0xb9, 0x71, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x47, 0xd3, 0x63, 0xc2, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x98, 0x93, 0x22, 0xfd, 0x16, 0x00, 0x00,
			0x00, 0x01, 0xe7, 0x53, 0x00, 0xe1, 0x64, 0x00, 0x00,
			0x00, 0x3a, 0xfd, 0x13, 0x00, 0x9e, 0xb5, 0x00, 0x00,
			0x00, 0x8b, 0xcf, 0x07, 0x07, 0x5f, 0xf8, 0x0d, 0x00,
			0x00, 0xdc, 0xff, 0xff, 0xff, 0xff, 0xff, 0x57, 0x00,
			0x2d, 0xff, 0x2c, 0x00, 0x00, 0x00, 0xbd, 0xa8, 0x00,
			0x7e, 0xd7, 0x00, 0x00, 0x00, 0x00, 0x69, 0xf2, 0x07,
			0xcf, 0x82, 0x00, 0x00, 0x00, 0x00, 0x18, 0xfd, 0x4a,
		}
	default:
		panic("no data")
	}
}
